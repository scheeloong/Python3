...
# This is the base case to the problem specific classes such as 
# waterjugs, eightPuzzle, and bicycle 
# It is fully implemented so do not touch this file. 
...
class StateSpace:
    ...
    n = 0 # the level of depth of the state space 
    ...
    def __init__(self, action, gval, parent):
        ...
        StateSpace.n = StateSpace.n + 1 #increment the depth for next state space
    ...
    def print_path(self):
        ...
        # While there are still parent's of s, append it to states 
        while s:
            states.append(s)
            s = s.parent
        # Now pop the appeneded state from the parent 
        states.pop().print_state()
        # While there are children 
        while states:
            print(" ==> ", end="") # end the print function with an empty string rather than a new line 
            # Pop the next children 
            states.pop().print_state()
        print("")
    ...
    def has_path_cycle(self):
        ...
        # Loop through all states upwards in the hierarchy 
        # and check if any of it is the hashable state already in the hash table 
        while s:
...
#----------------------------------------------------------
# CONTINUE HERE AFTER LEARNING BEST FIRST AND ASTAR 
...
_SUM_HG = 0 # Use gval+hval for comparison in priority queue 
_H = 1 # Use hval for comparison in priority queue 
_G = 2 # use gval for comparison in priority queue 
...
_CC_PATH = 1 # Path cycle checking (If there's a cycle in current path) 
_CC_FULL = 2 # Full cycle checking (If there is a cycle with respect to all paths explored) 
...
def _zero_hfn(state): # Heuristic function for uninformed search 
    '''Null heuristic (zero)'''
    return 0
...
class sNode:
    ...
    n = 0 # To  be the index of this node 
    lt_type = _SUM_HG # The comparison function for priority queue 
                      # compares the sum of h+g values 
    ...
    def __init__(self, state, hval):
        ...
        self.index = sNode.n
        sNode.n = sNode.n + 1 # always increment the index once given to a new node 

    def __lt__(self, other): # This is the default notation for comparison operators
                             # for the priority queue (heapq) data structure in python3 
        '''For astar and best first we must a priority queue for the
        ...
        # The sNode class does not follow the implementation guidelines 
...
class Open:
    ...
    def __init__(self, search_strategy):
        ...
        # Use priority queue for Best First using hvalue only as comparison 
        ...
        # Use priority queue for A* using g+h value as comparison 
...
# This class defines the search strategy used 
class SearchEngine:
    def __init__(self, strategy = 'depth_first', cc_level = 'default'):
        self.set_strategy(strategy, cc_level)
        self.trace = 0 # Set trace to level 0 for no tracking 
    ...
    def set_strategy(self, s, cc = 'default'):
        if not s in ['depth_first', 'breadth_first', 'best_first', 'astar']:
            ...
            if cc == 'default' :
                if s == 'depth_first' :
                    self.cycle_check = _CC_PATH # use path cycle checking for DFS for default
                else:
                    self.cycle_check = _CC_FULL  # use full cycle checking for anything NOT DFS for default 
    ...
    # Return the strategy to pick open Nodes as well as cycle checking in a string 
    ...
    ###INIT the Search
        ...
        #BEGIN TRACING
        # only begin tracing if tracing is set 
        if self.trace:
        ...
        # Create the openNodes with the strategy given 
        OPEN = Open(self.strategy)
        
        # create each nodes with the heuristic function given 
        ...
        if self.cycle_check == _CC_FULL:
            ...
            # Key is the hashable_state, value is the gValue  and the node 
            self.cc_dictionary[initState.hashable_state()] = initState.gval
        OPEN.insert(node)

    ###NOW do the search and return the result
        goal_node = self.searchOpen(OPEN, goal_fn, heur_fn) # The entire search happens here and returns the goal_node that was found 
        if goal_node:
    ...
    def searchOpen(self, OPEN, goal_fn, heur_fn):
        ...
        # If OPENset is empty but havent, found solution yet, 
        # will return false cause no solution was found 
        while not OPEN.empty():
            node = OPEN.extract() # Extract the next node depending on what it is 
